...
export default function DisplayedTask({ task }: TaskProps) {
  ...
  return (
    <View
      className="absolute z-50 flex w-full flex-row items-center gap-1"
        ...
        <Link href="/task-form" push asChild>
          ...
          >
            <View className="absolute bottom-0 left-0 right-0 top-0 w-full flex-row">
              ...
              <View
                className={`justify-left w-full flex-row rounded-e p-1 ${taskOpaqueColors[task.color]}`}
              >
                <Text className="font-bold text-black">{task.name}</Text>
              </View>
            </View>
          </TouchableWithoutFeedback>
        </Link>
